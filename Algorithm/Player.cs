...
namespace Algorithm
{
    ...
    class Player
    {
        ...
        private void AStar()
        {
            ...
            // 시작점 발견(예약 진행)
            open[PosY, PosX] = 0 + 10 * (Math.Abs(_board.DestY - PosY) + Math.Abs(_board.DestX - PosX)); // 상하좌우 이동 기존 비용에 10을 곱했으니까
            pq.Push(new PQNode() { F = 0 + 10 * (Math.Abs(_board.DestY - PosY) + Math.Abs(_board.DestX - PosX)), G = 0, Y = PosY, X = PosX });
        ...
        private void RightHand()
        {
            ...
            while (PosY != _board.DestY || PosX != _board.DestX)
            {
                ...
                else if (_board.Tile[PosY + frontY[_dir], PosX + frontX[_dir]] == Board.TileType.Empty) // 2.현재 바라보는 방향을 기준으로 전진할 수 있는지 확인
                {
                    // 앞으로 한 보 전진
                    PosY = PosY + frontY[_dir];
                    PosX = PosX + frontX[_dir];
